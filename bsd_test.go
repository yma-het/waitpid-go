...
func ProcessContainerExecResult(exitCode int, r io.Reader) (string, error) {
	out, err := io.ReadAll(r)
	if err != nil {
		return "", fmt.Errorf("failed to read from container otput: %w", err)
	}
	if exitCode != 0 {
		return "", fmt.Errorf("got non-zero exit code %d: %s", exitCode, string(out))
	}
...
func ExecGetLog(ctx context.Context, t *testing.T, container testcontainers.Container, cmd []string, options ...tcexec.ProcessOption) (string, error) {
	...
	i, r, err := container.Exec(ctx, cmd, options...)
	if err != nil {
		return "", fmt.Errorf("failed to exec: %w", err)
	}
	result, err := ProcessContainerExecResult(i, r)
	if err != nil {
		return "", err
	}
	return result, nil
...
func InstallGolang(ctx context.Context, t *testing.T, c testcontainers.Container) error {
	...
	_, err = ExecGetLog(ctx, t, c, []string{"tar", "-C", "/usr/local", "-xzf", "/tmp/go.tar.gz"})
	if err != nil {
	...
	//_, _, err = c.Exec(ctx, []string{"ln", "-s", "/usr/local/go/bin/go", "/usr/bin/go"})
	...
	//_, _, err = c.Exec(ctx, []string{"ln", "-s", "/usr/local/go/bin/gofmt", "/usr/bin/gofmt"})
	if err != nil {
